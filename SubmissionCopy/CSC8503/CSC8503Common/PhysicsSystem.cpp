...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	float impusleForce = Vector3::Dot(contactVelocity, p.normal);
	...
	float j = (-(1.0f + cRestitution) * impusleForce) /
...
void PhysicsSystem::IntegrateVelocity(float dt) {
	...
	std::vector<GameObject*>::const_iterator last;
	gameWorld.GetObjectIterators(first, last);
	float frameLinearDamping = 1.0f - (0.5f * dt);
...
void PhysicsSystem::OutputCollisionToConsole(GameObject* objA, GameObject* objB) {
	std::cout << "Collision between " << objA->GetName()
		<< " and " << objB->GetName() << std::endl;
}